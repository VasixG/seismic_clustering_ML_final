...
class SavePredictionPCloud:
    ...
    def __init__(self, save_fold, file_name, bolvanka_path, 
        ...
        self._inspect_save_fold()
        self.file_name = file_name
        # self.if_save_segy = save_segy
        self.segment_res = np.load(segment_res_path)
        if self.segment_res.dtype == 'float64':
            self.segment_res = self.segment_res.astype('float32')
    ...
    def insert_res_to_xr(self):
        ...
        if self.segment_res.dtype == 'float32':
            print('Change bolvanka xr dtype to float32')
            self.results_xr[var_name] = self.results_xr[var_name].astype(np.float32)
